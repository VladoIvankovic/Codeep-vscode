...
(() => {
  ...
  var btnPersonality = document.getElementById("btn-personality");
  ...
  var personalityLabelEl = document.getElementById("personality-label");
  ...
  function makeMetaItem(label, value, title) {
    const item = document.createElement("div");
  ...
  function makePersonalityButton(label, command, icon, primary = false) {
  ...
  function renderPersonalitySection() {
    ...
    if (!state.personalitiesLoaded) {
      ...
      state.personalities.forEach((personality) => {
        ...
        option.textContent = personality.available ? personality.displayName : `${personality.displayName} \u2014 unavailable here`;
      ...
      badge.textContent = active ? active.source === "builtin" ? "Built-in" : active.source === "project" ? "Project" : "Synced" : "Default";
      ...
      const tools = active ? active.restrictTools ? active.tools.length > 0 ? active.tools.join(", ") : "None" : "Unrestricted" : "Unrestricted";
      const scope = active?.scope || "All projects";
      const scopeTitle = active?.scope === "Selected projects" && active.projects.length > 0 ? active.projects.join(", ") : void 0;
      metadata.append(
      ...
      if (active && !active.structured) {
        ...
        legacyCopy.textContent = active.source === "builtin" ? "Built-in personality: model, tools, and scope remain unrestricted." : "Legacy prompt: model, tools, and scope remain unrestricted.";
    ...
    footer.textContent = state.personalitySource === "rpc" ? "Connected to Codeep CLI \xB7 runtime rules are enforced by the core agent." : "Compatible mode \xB7 reads shared project and ~/.codeep personality files.";
  ...
  window.addEventListener("message", (event) => {
    ...
    switch (msg.type) {
      ...
      case "personalities": {
        ...
        btnPersonality.title = active ? `Custom bot: ${active.displayName}. Click to switch.` : "Using default Codeep. Click to choose a custom bot.";
        if (settingsPanelEl.style.display !== "none")
          renderSettingsPanel();
        break;
      }
